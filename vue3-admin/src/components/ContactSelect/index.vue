...
interface OpenOptions {
  ...
  /** 不可选中的部门/人员（按 type-id 判定，如已加入巡查组的老师） */
...
function isSelectable(data: OrgTreeNode): boolean {
  if (disabledKeys.value.has(`${data.type}-${data.id}`)) return false
...
function open(options?: OpenOptions): Promise<SelectedItem[] | null> {
  ...
  disabledKeys.value = new Set((options?.disabled ?? []).map(item => `${item.type}-${item.id}`))
  treeKey.value += 1
  visible.value = true
...
<template>
  ...
  >
    <div class="flex h-[55vh]">
      <div class="border-border-lighter flex w-2/3 shrink-0 flex-col overflow-hidden border-r pr-4">
            ...
            <template #default="{ data }">
              ...
              >
                ...
                <IconEpCheck
                  ...
                  class="text-primary ml-1.5 shrink-0 text-base"
                />
              </div>
            </template>
          </el-tree>
        ...
        <ul class="scroll-thin m-0 flex-1 list-none overflow-y-auto p-0">
          ...
          >
            <div class="flex min-w-0 flex-1 items-center">
              <span class="text-primary mr-2 flex shrink-0 items-center text-[15px]">
                <IconEpOfficeBuilding v-if="item.type === 1" />
                <IconEpAvatar v-else />
              </span>
              <span
                class="text-text-primary mr-2 overflow-hidden text-sm text-ellipsis whitespace-nowrap"
              >
                {{ item.name }}
              </span>
              <span
                class="bg-primary-light-9 text-primary shrink-0 rounded px-1.5 py-0.5 text-[11px]"
              >
                {{ item.type === 1 ? '部门' : '人员' }}
              </span>
            </div>
          ...
          <li
            v-if="!selectedList.length"
            class="text-text-placeholder py-8 text-center text-[13px]"
          >
            暂无选择
          </li>
        </ul>
