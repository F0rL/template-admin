...
export function registerSysUserMock(mock: MockAdapter) {
  ...
  mock.onGet('/api/SysUser/GetUserEntity').reply(config => {
    ...
    return [
      ...
      makeResp({
        ...
        sysRoleUsers: user.roleIds.map(rid => ({
          roleId: rid,
          roleName: ROLE_NAMES[rid] || '未知角色',
        })),
  ...
  mock.onPost('/api/SysFile/SysFileUpload').reply(config => {
  ...
  mock.onGet('/api/WxWork/GetOrgTree').reply(config => {
    ...
    if (departmentId === '0') {
      return [
        200,
        makeResp(
          ROOT_DEPT_IDS.map(id => ({ id, name: ORG_DEPTS[id].name, type: 1, isLeaf: false })),
        ),
      ]
    }
