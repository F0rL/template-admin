...
| 11  | 共享组件   | ProTable（antd Table 薄封装）+ SelectIcon（Phase 2 落地）  | ADR-0011 |
...
- 与用户确认首个系统管理页面（建议 `system/user`：筛选 + 分页表格 + 弹窗表单 + 树选择 + 上传，覆盖面最全）完整实现
- 随之落地：ProTable、SelectIcon、useDialogForm、ECharts（dashboard 用）、docs/{components, page-conventions}.md、对应 API + mock

### Phase 3：逐页完善

- 按 Phase 2 定下的模板逐个补齐 role / menu / org / log / dashboard 真实页面与对应 API + mock
...
### 6.7 共享组件（Phase 2 落地，设计先行）

- **ProTable**：antd Table 薄封装——补齐 valueEnum 字典渲染、selection/expand/tag 列语法糖、`auto-height` 满高（flex + 内部滚动）、默认 `rowKey="id"`；其余 props 原样透传
- **SelectIcon**：Popover + 双标签页（antd icons / ri）图标选择器，受控 value 为字符串
- **useDialogForm**：弹窗表单 hook（open / close / pending / 提交成功回调失效缓存）
...
1. 首个模板页选哪个（建议 `system/user`）？——Phase 2 开始时与用户确认
