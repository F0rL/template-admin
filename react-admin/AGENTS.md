...
React 后台管理范本，用于派生新的 React 后台项目。与 vue3-admin（Vue 版）互为独立对等物：目录、代码、文档完全独立，不共享任何文件。内置：登录（账号密码 + 扫码 tab）、系统管理菜单骨架（占位页）、错误页、HTTP/queryClient/stores/icons/layouts 等共享层、mock 完整登录链路。不含业务模块。
...
- Phase 1 仅 auth 模块带 mock；新业务模块默认不使用 mock。
...
| 页面代码规范 | docs/page-conventions.md（Phase 2 落地） | 编写/修改页面（暂以 views 现有页面为准）  |
| 共享组件     | docs/components.md（Phase 2 落地）       | 使用共享组件（暂无共享组件）              |
