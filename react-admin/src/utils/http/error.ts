...
import { navigate, getPathname } from '@/router/navigate'
...
export function handleBusinessError(res: ApiResponse<unknown>) {
  ...
  if (res.code === 401 && getPathname() !== '/login') {
...
function handleUnauthorized(msg = '登录已过期，请重新登录') {
  ...
  message.error(msg)
  // 非组件上下文跳转经 navigate 桥（App.tsx 内 <NavigateBridge/> 注入）
  navigate('/login')
  isReLogging = false
}
...
export function handleNetworkError(error: unknown): void {
  // 请求取消（react-query 组件卸载时自动 abort signal → axios CanceledError）
  // 静默处理，不弹 toast
