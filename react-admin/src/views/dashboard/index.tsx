/**
 * 首页 — 静态数据示例图表（骨架示例页）
 * ---------------------------------------------
 * 对应 vue3-admin views/dashboard/index.vue：折线 / 柱状 / 饼图三例，
 * 演示 components/EChart 用法（option 为普通对象，图表按需注册见 @/lib/echarts）。
 * 配色取自 antd token（theme.useToken），随主题切换；接入真实数据时把静态常量
 * 换成 useQuery 结果即可（参考系统管理页面写法）。
 */
import { theme } from 'antd'
import type { EChartsOption } from 'echarts'
import EChart from '@/components/EChart'

export default function DashboardPage() {
  const { token } = theme.useToken()

  const axisStyle = { splitLine: { lineStyle: { color: token.colorSplit } } }

  // 折线图：月度访问趋势
  const lineOption: EChartsOption = {
    tooltip: { trigger: 'axis' },
    grid: { left: 48, right: 24, bottom: 28, top: 40 },
    legend: { top: 4 },
    xAxis: {
      type: 'category',
      boundaryGap: false,
      data: ['1月', '2月', '3月', '4月', '5月', '6月'],
    },
    yAxis: { type: 'value', ...axisStyle },
    series: [
      {
        name: '访问量',
        type: 'line',
        smooth: true,
        data: [820, 932, 901, 1290, 1330, 1520],
        areaStyle: { opacity: 0.08 },
        itemStyle: { color: token.colorPrimary },
      },
      {
        name: '活跃用户',
        type: 'line',
        smooth: true,
        data: [620, 712, 681, 890, 930, 1120],
        areaStyle: { opacity: 0.08 },
        itemStyle: { color: token.colorSuccess },
      },
    ],
  }

  // 柱状图：模块使用统计
  const barOption: EChartsOption = {
    tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' } },
    grid: { left: 48, right: 24, bottom: 28, top: 40 },
    legend: { top: 4 },
    xAxis: { type: 'category', data: ['账户管理', '角色管理', '菜单管理', '组织架构', '日志管理'] },
    yAxis: { type: 'value', ...axisStyle },
    series: [
      {
        name: '操作次数',
        type: 'bar',
        barMaxWidth: 32,
        data: [320, 254, 180, 96, 512],
        itemStyle: { color: token.colorPrimary, borderRadius: [4, 4, 0, 0] },
      },
    ],
  }

  // 饼图：浏览器占比
  const pieOption: EChartsOption = {
    tooltip: { trigger: 'item', formatter: '{b}: {c} ({d}%)' },
    legend: { bottom: 0 },
    series: [
      {
        name: '浏览器占比',
        type: 'pie',
        radius: ['40%', '65%'],
        itemStyle: { borderRadius: 6, borderColor: token.colorBgContainer, borderWidth: 2 },
        label: { show: false },
        data: [
          { value: 1048, name: 'Chrome' },
          { value: 735, name: 'Edge' },
          { value: 580, name: 'Firefox' },
          { value: 484, name: 'Safari' },
        ],
      },
    ],
  }

  return (
    <div className="flex flex-col gap-4">
      <div className="panel-card">
        <div className="mb-4 font-semibold">访问趋势</div>
        <div className="h-80 w-full">
          <EChart option={lineOption} />
        </div>
      </div>

      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        <div className="panel-card">
          <div className="mb-4 font-semibold">模块使用统计</div>
          <div className="h-80 w-full">
            <EChart option={barOption} />
          </div>
        </div>
        <div className="panel-card">
          <div className="mb-4 font-semibold">浏览器占比</div>
          <div className="h-80 w-full">
            <EChart option={pieOption} />
          </div>
        </div>
      </div>
    </div>
  )
}
