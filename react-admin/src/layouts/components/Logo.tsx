...
export default function Logo() {
  ...
  return (
    ...
    >
      <div className="text-text-inverse flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-blue-500 to-indigo-600 text-sm font-bold">
        {config.APP_TITLE.slice(0, 1)}
      </div>
      {!sidebarIconOnly && (
        <span className="truncate text-base font-bold text-gray-900">{config.APP_TITLE}</span>
      )}
    </button>
  )
}
